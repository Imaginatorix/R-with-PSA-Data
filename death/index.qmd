---
title: "Deaths in the Philippines (2021)"
author: "Imaginatorix"
format:
  html:
    code-fold: true
...
## Questions with Possible Answer based on Visualization
...
site_death <- read_excel("death_data.xlsx", 
...
# Get geographic data
for (i in 1:3){
  gadm(country="PHL", path="./geo/", level=i)
}
geo_data_level_1 <- readRDS(paste(getwd(), "/geo/gadm/gadm41_PHL_1_pk.rds", sep="")) # Province
geo_data_level_2 <- readRDS(paste(getwd(), "/geo/gadm/gadm41_PHL_2_pk.rds", sep="")) # Municipality/City
geo_data_level_3 <- readRDS(paste(getwd(), "/geo/gadm/gadm41_PHL_3_pk.rds", sep="")) # Village
...
(1) Can you find anomalies on the number of deaths on site per region? Explain possible reason.
...
- What happened at Cavite???
  - Why are there more deaths there?
  - Are there more hospitals there?
  - Was there an isolation event in which deaths accumulated in that place only? Like COVID
...
(1) Difference in number of deaths in attended vs not attended?
```{r}
...
(2) Compare death difference each profession that attended
```{r}
...
(3) Can you find anomalies on the deaths by attention per region?
